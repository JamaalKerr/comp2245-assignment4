...
// Define the array of superheroes
$superheroes = [
    [
        "id" => 1,
        "name" => "Steve Rogers",
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    // If the search query is empty, return all superheroes
    if (empty($searchQuery)) {
        ...
        foreach ($superheroes as $superhero) {
            if (
                stripos($superhero['name'], $searchQuery) !== false ||
                stripos($superhero['alias'], $searchQuery) !== false
            ) {
                $foundSuperheroes[] = $superhero;
            }
        }

        // Return the matching superheroes as JSON
        header('Content-Type: application/json'); // Set the content type to JSON
        echo json_encode($foundSuperheroes);
    }
}
